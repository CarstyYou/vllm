...
- **cute kernel 低并发劣势实测 = kernel 特有非 recipe**：35B 单卡 cc=1 cute_fp8/cute_mxfp8_32
  比 triton 慢 ~35%，但**同 recipe 的 dg_mxfp8_32（185）≈ triton**——cc=1 cute_mxfp8_32 只 126，
  证明是 cute kernel 的小 batch 效率惩罚，非 K32 recipe；cc=128 全收敛（cute 反超）。归因移交 [task_06]。
- **dg_ue8m0（DeepGEMM UE8M0 K128）在 DSv4 TP4 + cc=128 可复现 CUDA illegal memory access**：
  8k input × 512 prompts 高并发触发，两次独立 run 均崩（非 transient）；cc≤64 正常。
  该 cell 记为已知失败（baseline/参考列，非 cute），empty json 已删，不再重试。
