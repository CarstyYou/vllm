#!/bin/bash
# usage: run_aime_mc.sh <model_dir> <tp> <backend|auto> [port]
...
python -m vllm.entrypoints.openai.api_server \
  ...
  --max-model-len 36864 --enable-prefix-caching \
...
lm_eval --model local-completions \
  --model_args "base_url=http://localhost:$PORT/v1/completions,model=evalmodel,tokenizer=$MODEL,num_concurrent=32,max_retries=3,max_length=36864,timeout=7200" \
  --tasks aime24,aime25 --seed 42 \
  ${EXTRA_LM_ARGS:-} --output_path $OUT_DIR/aime_${MTAG} \
  2>&1 | tee $OUT_DIR/aime_${MTAG}.log | tail -40
echo "AIME_DONE $MTAG"
